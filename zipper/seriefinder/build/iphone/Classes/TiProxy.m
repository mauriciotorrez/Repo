...
//Rare exceptions to indicate a bug in the _seriefinder code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_seriefinder = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_seriefinder]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.seriefinder.module"
